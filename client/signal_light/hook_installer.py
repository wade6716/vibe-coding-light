...
ANTIGRAVITY_HOOK_SCRIPT = PROJECT_ROOT / "scripts" / "antigravity-signal-hook"
...
def supported_agents(home: Path | None = None) -> dict[str, AgentSpec]:
    ...
    return {
        ...
        "antigravity": AgentSpec(
            key="antigravity",
            name="Antigravity",
            config_path=home_dir / ".gemini" / "config" / "hooks.json",
            hook_script=ANTIGRAVITY_HOOK_SCRIPT,
            events=ANTIGRAVITY_EVENTS,
            passes_event_arg=True,
            uses_matcher=True,
            root_key="signal-light",
        ),
    }
...
def inspect_agent(spec: AgentSpec) -> AgentStatus:
    ...
    hooks = config.get(spec.root_key) if isinstance(config, dict) else None
    if not isinstance(hooks, dict):
        return AgentStatus(
            ...
            missing_events=tuple(spec.events),
            broken_events=(),
            message=f"{spec.root_key} missing",
        )
...
def install_agent(spec: AgentSpec, *, backup: bool = True) -> InstallResult:
    ...
    hooks = config.setdefault(spec.root_key, {})
    if not isinstance(hooks, dict):
        hooks = {}
        config[spec.root_key] = hooks
...
def _event_has_expected_hook(entries: object, spec: AgentSpec, event: str, timeout: int) -> bool:
    ...
    is_flat = (spec.key == "antigravity" and event in {"PreInvocation", "PostInvocation", "Stop"})
    ...
    for item in entries:
        ...
        if is_flat:
            if (
                ...
                and item.get("timeout") == timeout
            ):
                return True
            continue

        if spec.uses_matcher:
            expected_matcher = "*" if spec.key == "antigravity" else ""
            if item.get("matcher") != expected_matcher:
                continue
...
def _merge_event_groups(existing_entries: object, spec: AgentSpec, event: str, timeout: int) -> list[object]:
    ...
    for item in existing_entries:
        if is_flat:
            if isinstance(item, dict) and item.get("type") == "command" and _is_signal_light_command(item.get("command"), spec):
                if not replaced:
            ...
            replacement_group, cleaned_group, had_signal_light_hook = _replace_signal_light_hooks(item, spec, replacement)
...
def _hook_group(spec: AgentSpec, event: str, timeout: int) -> dict[str, object]:
    ...
    if spec.uses_matcher:
        group["matcher"] = "*" if spec.key == "antigravity" else ""
    return group
